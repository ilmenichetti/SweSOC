...
# 3 The dataset and the steady state assumption
...
The linear models already implemented in SoilR as predefined function are:  
...
Plus some more generic models, one of which we will also include in the comparison, in particular:   

* ThreepFeedbackModel  
  
And the baseline, the simplest possible model:  

* Olson's single pool model  
  
We will include in the comparison also models specifically developed for this study, in particular  

* Porous  
  
We classify them based on their characheristics, considering:

* Serial: the pools are all connected in a cascade, with one unique pathway for C molecules other than being respured
* Parallel: some pools are defining parallel pathways for C molecules
* Inert: the model assumes one proper "inert" pool, with some C molecules not reacting
* Feedback: the model assumes some patways bacwards
* Nonlinear: there are nonlinearities in the model

```{r echo=FALSE}

model_names<-c("Olson","ICBM", "Century", "Yasso", "Yasso07", "RothC", "Porous", "3PFeedback")

venn_models<-mat.or.vec(length(model_names), 5)
rownames(venn_models)<-model_names
colnames(venn_models)<-c("Serial", "Parallel", "Inert", "Feedback", "Nonlinear")

venn_models[1,]=c(1,0,0,0,0)
venn_models[2,]=c(1,0,0,0,0)
venn_models[3,]=c(0,1,0,0,0)
venn_models[4,]=c(1,0,0,0,0)
venn_models[5,]=c(1,0,0,0,0)
venn_models[6,]=c(1,0,0,1,0)
venn_models[6,]=c(1,0,0,0,1)
venn_models[6,]=c(1,0,0,1,0)


knitr::kable(venn_models)
```
...
### 4.1.1 Serial (Cascade) models

### 4.1.2 Parallel models

### 4.1.3 Models with inert pool

### 4.1.4 Feedback models
...
# 5 Comparing models
## Model Complexity
Model complexity can be defined in various ways, and have very different meanings. 
